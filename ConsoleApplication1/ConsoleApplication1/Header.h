				...
				*/
				...
				if (!getMarketData || analyseCase == 0){
					...
					if (doPriipsStress){
						...
						sprintf(lineBuffer, "%s%s%.5lf%s", lineBuffer, "ESvol='", doPriipsVol ? scaledVol : esVol, doPriipsVol ? "'/sqrt(duration) +'0" : "");   // pesky way to pick up previously saved realWorld duration
						if (doPriipsVol){
